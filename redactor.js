...
(function($)
{
    ...
    Redactor.fn = $.Redactor.prototype = {
        ...
        init: function(el, options)
        {
            ...
            if (this.opts.linebreaks === false)
            {
                ...
                // Fix bug with <p>&#x200b;</p> when in linebreaks mode - caused the infamous jumping cursor bug: https://trello.com/c/PgKAzLFK/1087-jumping-cursor-bug
                // This also caused problems if you inserted an inline image, then put the cursor in front of the image and hit enter - the image disappeared.  - bjones 6/22/2015
                this.opts.emptyHtml = '&#x200b;'
            }
        ...
        syncClean: function(html)
        {
            ...
            // special characters
            //Unclear why we were replacing &amp; with &, but it caused headaches for clients: https://secure.helpscout.net/conversation/109894841/129492/
            //html = html.replace(/&amp;/gi, '&');
        ...
        buildEventKeydown: function(e)
        {
            ...
            if (this.browser('mozilla')  && "getSelection" in window && "modify" in window.getSelection())  {
                //firefox cmd+left/Cmd+right browser back/forward fix - http://joshrhoderick.wordpress.com/2010/05/05/how-firefoxs-command-key-bug-kills-usability-on-the-mac/
            ...
            if (ctrl && !e.shiftKey) this.shortcuts(e, key);
                ...
                if (key === 55) this.shortcutsLoad(e, 'insertorderedlist'); // Ctrl + &
                else if (key === 56) this.shortcutsLoad(e, 'insertunorderedlist'); // Ctrl + *
            }
            ...
            if (key == 32)
            {
                /*
                 * a
                 * Note: in Firefox v39, began to see a bug where spacebar followed by backspace caused cursor to disappear. Documented here: https://trello.com/c/f1uC44RC/1168-firefox-39-cursor-disappears-after-a-sequence-of-hitting-backspace-and-enter
                 * Figured out that the problem had to do with selection saving when spacebar was pressed (after a linebreak had been entered too for some reason).  Firefox simply blew up, leaving editor in a weird state where one could not type, it did
                 * not have focus, BUT keydown/keyup events were still firing.
                 * A workaround is to only make bufferSet() save the selection if a non-collapsed selection has been made (i.e. something really is selected).
                 */
                range = this.getRange();
                var selectionSave = range && !range.collapsed;
                this.bufferSet(undefined, selectionSave);
            }
            ...
            else if (key === this.keyCode.ENTER && (e.shiftKey)) // Shift+Enter or Ctrl+Enter
            {
                this.bufferSet();
        ...
        shortcuts: function(e, key)
        {
            ...
            else if (key === 66 && !e.altKey) this.shortcutsLoad(e, 'bold'); // Ctrl + b
            else if (key === 73 && !e.altKey) this.shortcutsLoad(e, 'italic'); // Ctrl + i
            else if (key === 85 && !e.altKey) this.shortcutsLoad(e, 'underline'); // Ctrl + u
        },
        ...
        toggleCode: function(direct)
        {
            ...
            //close the link tooltips when switching to code mode
        ...
        toolbarBuild: function()
        {
            ...
            $.extend(this.opts.toolbar, this.opts.toolbarCustom);
        ...
        execUnlink: function(cmd, param)
        {
            ...
            if (link)
            {
                //split the text into a separate text node for each character - this will prevent redactor from detecting that the text nodes are in fact a single URL - bjones 12-19-2014
                var count = link.childNodes.length;
                for ( var i = 0; i < count; i++ ) {
                    var node = link.childNodes[i];
                    if (node && node.nodeType == 3 && node.length > 1 ) {
                        node.splitText(1)
                        count = link.childNodes.length;
                    }
                }
        ...
        cleanFinish: function(code)
        {
            ...
            /**
             * This code was consolidating consecutive linebreaks even inside pre/code/script tags.
             * This was frustrating, so stole this code from cleanRemoveSpaces to prevent the formatting here from applying to
             * pre/code/script tags.  -bjones 8/5/2015
             * */
            var buffer = [];
            var matches = code.match(/<(pre|style|script)(.*?)>([\w\W]*?)<\/(pre|style|script)>/gi) || [];
            $.each(matches, function(i, s) {
                code = code.replace(s, 'buffer_' + i);
                buffer.push(s);
            });
            ...
            code = this.cleanReplacer(code, buffer);
        ...
        insertLineBreak: function(twice)
        {
            ...
            var parent = this.getParent();
            var parentIsAnchor = parent && parent.tagName === 'A';

            if (this.browser('mozilla') && !parentIsAnchor)
            {
                // This appears to be necessary to solve the stupid old FF newline has no height bug.
                // However, it causes the newline at end of link causes next line to be inside the link bug, so _don't_ do it if the parent is an anchor tag
            ...
            else
            {
                ...
                {
                    // This was originally added to solve bug where if you clicked just to the right of the very last character in a link which was followed by a newline
                    // and then hit enter, the linebreak would be _inside_ the <a>, and the following line would be part of the URL.
                    // the first fix for this was to insert this #selection-marker-1, which solved that bug.  However, this appears to have caused Fix cursor jumping bug whose steps are detailed in https://trello.com/c/KYNO3QH2
                    // which is another instance of cursor jumping bug.  This occurred because the #selection-marker-1 did not have the .redactor-selection-marker and therefore did _not_ get cleaned up
                    // when it needed to, leading to 2 concurrent #selection-marker-1's, which caused the cursor to jump to the first one, which was added here.
                    // Adding that class now (7/24/2015) causes it to get cleaned up and cursor appears to function normally.  -bjones

                    this.linkObserverTooltipClose(false);
                    if ( this.isCaretAtEndOfNode(parent) ) {
                        this.selectionRemoveMarkers();
                        var node = $('<span id="selection-marker-1" class="redactor-selection-marker">' + this.opts.invisibleSpace + '</span>', this.document)[0];
                        $(parent).after(node);
        ...
        isCaretAtEndOfNode: function(node) {
            ...
            if ( range && node ) {
                ...
                //zero-width space character jacking us up again - bjones 2/10/15
                var postText = postRange.cloneContents()
                postText = postText.textContent.replace(/[\u200B-\u200D\uFEFF]/g, '');
                caretAtEnd = (postText.length === 0)
            }
            return caretAtEnd;
        },
        ...
        pasteClean: function(html)
        {
            ...
            // Note: IE does not support pasteEvent.clipboardData.  It instead has window.clipboardData, and it does not appear that you can access the raw data from it.
            // However, at least in IE 11, the browser will turn a pasted clipboard image data into an <img> tag with a src of "data:image/png,[base64_data]".
            // This is what firefox does as well, so treat IE it like firefox in this scenario and parse it after the fact to perform the upload
            if (this.browser('mozilla') || (this.browser('msie') && window.clipboardData))
            {
        ...
        linkProcess: function()
        {
            ...
            // mailto
            // don't prepend mailto: in cases where query string or hash contains an "@" - - bjones 7/22/2014
            if (link.search('@') != -1 && link.search(/\?/) == -1 && link.search('#') == -1)
            {
                link = 'mailto:' + link;
            }
            else if (link.search(/^skype\:/i) !== -1) {
                // Allow `skype:` links. cdavis convo #104749
                if (link.search(/\?call$/i) == -1) {
                    // Ensure that the url has the important call param
                    link = link + '?call';
                }
            }
            else if (link.search(/^tel\:/i) !== -1) {
                // Allow `tel:` link. cdavis convo #104749
            }
            else if (link.search('http') == -1 && link.search('://') != -1) {
                // has a custom protocol - don't mess with it - bjones 7/16/2014 - see convo #95616
            }
            // url, not anchor
            else if (link.search('#') != 0)
            {
                ...
                //Fix catastrophic backtracking - stole this fix from latest Redactor source
        ...
        // fix a blowup we've been getting when you try to insert or upload images/files when the editor is empty. See https://secure.helpscout.net/conversation/47603627/101513/ - bjones 10/6/14
        fixInsertIntoEmptyEditorBug: function() {
            if ( !this.get() ) {
                this.$editor.html(this.opts.emptyHtml);
                var children = this.$editor.children();
                children.length && this.selectionElement(children[0]);
            }
        },
        fileCallback: function(json)
        {
            ...
            if (json !== false)
            {
                ...
                this.fixInsertIntoEmptyEditorBug();
        ...
        imageCallbackLink: function()
        {
            ...
            if (val !== '')
            {
                ...
                var $img = this.imageInsert(data, true);
                this.callback("imageLinkInsert", $img);
            }
        ...
        modalInit: function(title, content, width, callback)
        {
            ...
            {
                this.$modal = $redactorModal = $('<div id="redactor_modal" style="display: none;"><div id="redactor_modal_close"><i class="icon-close2"></i></div><header id="redactor_modal_header"></header><div id="redactor_modal_inner"></div></div>');
            ...
            if ($redactor_tabs.length )
            {
                ...
                $redactor_tabs.find('a').each(function(i, s)
                {
                    ...
                    $(s).on('click', function(e)
                    {
                        ...
                        $('.redactor_tab').hide();
                        //custom mod - focus on first input in the tab when opened - bjones 6/22/2015
                        $('#redactor_tab' + i ).show().find('input').focus();
        ...
        dragUploadAjax: function(url, file, directupload, progress, e, uploadParam)
        {
            ...
            var options = {
                ...
                success: $.proxy(function(data)
                {
                    ...
                    if (directupload)
                    {
                        ...
                        //insertNodeToCaretPositionFromPoint() doesn't work right with an empty editor. instead, call inserthtml. - bjones 10/6/14
                        if ( !this.get() ) {
                            this.fixInsertIntoEmptyEditorBug();
                            this.execCommand('inserthtml', $('<div/>').append($img[0]).html(), false);
                        } else {
                ...
                error: $.proxy(function(xhr, textStatus) {
                    ...
                    if ( xhr && xhr.responseText ) {
                        try {
                            ...
                            console.log("Error: response was not JSON: ", xhr.responseText );
                        }
                    }
                    this.callback('imageUploadError', json);
                }, this)
            ...
            if (!directupload)
            {
                ...
                /*
                 Warning: globally overriding the xhr setting with ajaxSetup is a *terrible* idea and caused an issue with comet, because if this request was outstanding, when comet ran next,
                 it would reuse the same xhr object, causing the file upload to be aborted.  I moved this xhr setting into the $.ajax() call to prevent this from happening. -bjones 10/8/14
                 $.ajaxSetup({
                 xhr: function () { return xhr; }
                 });*/

                options.xhr = function () { return xhr; };
            }
            window.redactorXhr = $.ajax(options);
        },
    ...
    $.Redactor.fn.formatLinkify = function(protocol, convertLinks, convertImageLinks, convertVideoLinks, linkSize)
    {
        ...
        {
            var n = childNodes[i];
            var isInPreTag = $(n).closest('pre, code, [style*="white-space: pre"]').length > 0;
            if ( !isInPreTag ) {
                if (n.nodeType === 3)
                {
                    var html = n.nodeValue;

                    //replace stupid zero width character
                    html = html.replace(/[\u200B-\u200D\uFEFF]/g, '');
                    ...
                    if (convertLinks && html && (html.match(url1) || html.match(url2)))
                    {
                        ...
                        while (found)
                        {
                            ...
                            //custom - only modify the URL *name*, don't truncate the URL itself, that's stupid
                            var title = href;
                            if (found && href && href.length > linkSize)
                            {
                                title = href.substring(0, linkSize) + '...';
                            }
                            ...
                            if (found && href)
                            {
                                //to handle URLs which may have $ characters in them, need to escape $ -> $$ to prevent $1 from getting treated as a backreference. See http://gotofritz.net/blog/code-snippets/escaping-in-replace-strings-in-javascript/
                                var escapedBackReferences = title.replace('$', '$$$');
                                if (url == url1)
                                {
                                    html = html.replace(url1, '$1<a href="' + protocol + '$2">' + $.trim(escapedBackReferences) + '</a>$3$4')
                                }
                                else
                                {
                                    html = html.replace(url2, '$1<a href="$2">' + $.trim(escapedBackReferences) + '</a>$5$6');
                                }
                            }
